...
class DoctorAnomalyHandler(Node):
    def __init__(self):
        ...
        self.update_outputs()
        self.get_logger().info('[doctor_anomaly_handler] ready (I/O wired)')
    ...
    def update_outputs(self):
        ...
        self.get_logger().info(f'[doctor_anomaly_handler] allow_targets={allow} banner="{banner}"')

def main(args=None):
    rclpy.init(args=args)
    node = DoctorAnomalyHandler()
    try:
        rclpy.spin(node)
