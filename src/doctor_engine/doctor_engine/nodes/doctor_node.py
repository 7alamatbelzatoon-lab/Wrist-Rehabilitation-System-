...
class DoctorNode(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(Float32, '/doctor/target_position', 10)
        #self.publisher_ = self.create_publisher(Float32, 'target_position', 10)

        # Predefined sequence of angles to test the adaptive system
        self.angle_sequence = [0.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0, 55.0, 60.0, 65.0, 70.0, 70.0, 65.0, 60.0, 55.0, 50.0, 45.0, 40.0,
                                35.0, 30.0, 25.0, 20.0, 15.0, 10.0, 5.0, 0.0]

        self.index = 0

        self.timer = self.create_timer(1.0, self.publish_target_position)
        #self.target = 30.0  # degrees
    ...
    def publish_target_position(self):
        if self.index < len(self.angle_sequence):
            ...
            self.destroy_node()  # Stop the node after finishing     
